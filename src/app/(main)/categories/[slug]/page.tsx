...
// Import data/hàm từ HomePage
import { categoryData, productData, formatPrice } from "@/app/(main)/page";

export default function CategoryDetail() {
  const { slug } = useParams() as { slug: string };

  // Lọc danh mục chính dựa trên slug
  const categoryGroup = categoryData.find(
    (group) =>
      group.categoryLink.includes(slug) ||
      group.categories.some((cat) => cat.link.includes(slug))
  );
          ...
          <div className="text-black flex flex-wrap gap-2 mt-2">
                ...
                <div className="text-black absolute z-10 mt-1 w-64 bg-white border border-gray-200 rounded-md shadow-lg p-2">
                  <div className="mb-2">
                    ...
                    {[
                      ...
                      <Button key={t} variant="ghost" size="sm" className="w-full text-left">
                        {t}
                      </Button>
                    ))}
                  </div>
                  ...
                  <div className="mb-2">
                    ...
                    {["Điện thoại", "Tablet", "Laptop/ Macbook"].map((t) => (
                      <Button key={t} variant="ghost" size="sm" className="w-full text-left">
                        {t}
                      </Button>
                    ))}
                  ...
                  <div>
                    ...
                    {["Tenkeyless", "Mini-size", "Full-size"].map((t) => (
                      <Button key={t} variant="ghost" size="sm" className="w-full text-left">
                        {t}
                      </Button>
                    ))}
            ...
            <div className="relative">
              ...
              {isDropdownOpen.closure && (
                <div className="absolute z-10 mt-1 w-48 bg-white border border-gray-200 rounded-md shadow-lg">
                  {["Đóng sản phẩm 1", "Đóng sản phẩm 2"].map((t) => (
                    <Button key={t} variant="ghost" size="sm" className="w-full text-left">
                      {t}
                    </Button>
                  ))}
            ...
            <div className="relative">
              ...
              {isDropdownOpen.compatibility && (
                <div className="absolute z-10 mt-1 w-48 bg-white border border-gray-200 rounded-md shadow-lg">
                  {["Tương thích 1", "Tương thích 2"].map((t) => (
                    <Button key={t} variant="ghost" size="sm" className="w-full text-left">
                      {t}
                    </Button>
                  ))}
