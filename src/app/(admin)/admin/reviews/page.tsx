...
export default function ReviewsPage() {
  ...
  const filteredReviews = reviews.filter(r => {
    return (
      (!filter.product || r.productName.toLowerCase().includes(filter.product.toLowerCase())) &&
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <Card className="bg-muted">
              <CardContent className="pt-4">
                ...
                <div className="text-xl font-bold">{((positive / total) * 100).toFixed(0)}%</div>
              </CardContent>
            </Card>
            <Card className="bg-muted">
              <CardContent className="pt-4">
                <div className="text-muted-foreground">Tỷ lệ tiêu cực</div>
                <div className="text-xl font-bold">{((negative / total) * 100).toFixed(0)}%</div>
              </CardContent>
            </Card>
            <Card className="bg-muted">
              <CardContent className="pt-4">
        ...
        <CardContent>
          <div className="flex flex-wrap gap-2">
            ...
            <Select
              ...
              onValueChange={(v: string) => setFilter({ ...filter, status: v === 'all' ? '' : v })}
            >
              <SelectContent>
            ...
            <Select
              ...
              onValueChange={(v: string) => setFilter({ ...filter, rating: v === 'all' ? '' : v })}
            >
              <SelectContent>
                <SelectItem value="all">Tất cả điểm số</SelectItem>
                {[1, 2, 3, 4, 5].map(star => (
                  <SelectItem key={star} value={String(star)}>{star} sao</SelectItem>
                ))}
              </SelectContent>
            </Select>
            <Input
          ...
          <Table>
            ...
            <TableBody>
              {filteredReviews.map(r => (
                <TableRow key={r.id} className={r.rating <= 2 ? 'bg-destructive/10' : ''}>
                  ...
                  <TableCell>
                    ...
                    <a href={`/product/${r.sku}`} className="text-primary text-xs underline" target="_blank" rel="noopener noreferrer">Xem sản phẩm</a>
                  </TableCell>
                  <TableCell className="max-w-xs">
                    <div>{r.content}</div>
                    {r.adminReply && (
                      <div className="mt-2 text-xs text-green-600 border-t pt-1">Phản hồi: {r.adminReply}</div>
                    )}
                  </TableCell>
                  <TableCell className="text-center">
                    <Badge variant={r.rating <= 2 ? "destructive" : "default"}>{r.rating}★</Badge>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    ...
                    >
                      {statusMap[r.status as keyof typeof statusMap]}
                    </Badge>
                  </TableCell>
                  <TableCell className="space-y-1">
                    <div className="flex flex-wrap gap-1">
                      {r.status === 'pending' && (
                        <Button size="sm" variant="destructive" onClick={() => handleApprove(r.id)}>Phê duyệt</Button>
                      )}
                      {r.status !== 'hidden' && (
                        <Button size="sm" variant="secondary" onClick={() => handleHide(r.id)}>Ẩn</Button>
                      )}
                    </div>
              ...
              {filteredReviews.length === 0 && (
                <TableRow>
                  <TableCell colSpan={8} className="text-center text-muted-foreground py-4">Không có đánh giá nào phù hợp.</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
...
function ReplyBox({ review, onReply }: { review: any, onReply: (id: number, reply: string) => void }) {
