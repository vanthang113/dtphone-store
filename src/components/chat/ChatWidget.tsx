...
import { Menu, X, ThumbsUp, Paperclip, Smile } from 'lucide-react';
...
const ChatWidget = ({ onClose }: ChatWidgetProps) => {
  ...
  const messages: ChatMessage[] = [
    ...
    { id: 2, sender: 'user', content: '👍', timestamp: '6/17/2025, 9:08:05 PM' },
    {
      id: 3,
      sender: 'agent',
      content:
      ...
      menu: ['Tư vấn sản phẩm', 'Kiểm tra thông tin', 'Chat Zalo', 'Gặp tư vấn viên'],
    },
    { id: 4, sender: 'user', content: 'Kiểm tra thông tin', timestamp: '6/17/2025, 9:08:15 PM' },
    { id: 5, sender: 'user', content: 'Tư vấn sản phẩm', timestamp: '6/17/2025, 9:08:20 PM' },
    { id: 6, sender: 'agent', content: 'Dạ CellphoneS xin chào anh Lộc Trần Trân', timestamp: '6/17/2025, 9:08:25 PM' },
    { id: 7, sender: 'agent', content: 'Dạ không biết mình cần em hỗ trợ gì ạ', timestamp: '6/17/2025, 9:08:30 PM' },
  ];

  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    console.log('Tệp được tải lên:', file);
  };
    ...
    <div className="fixed inset-0 sm:inset-auto sm:bottom-4 sm:right-4 z-50 w-full sm:w-[400px] bg-white sm:rounded-lg shadow-lg flex flex-col h-full sm:h-[500px] sm:max-h-[80vh]">
      <div className="flex justify-between items-center p-4 border-b bg-[#00868B] text-white">
        <div className="flex items-center gap-3 ">
          <Avatar className="w-10 h-10">
            <AvatarImage src="/images/chibi_head.png" alt="Hồng Vân.CTV_CellphoneS" />
      ...
      {isMenuOpen && (
        <div className="absolute top-16 right-4 bg-white shadow-md rounded-md w-48">
          <ul className="py-1">
            {['Tư vấn sản phẩm', 'Kiểm tra thông tin', 'Chat Zalo', 'Gặp tư vấn viên'].map((item) => (
              <li key={item}>
                <Button variant="ghost" className="w-full text-left px-4 py-2 text-sm" onClick={() => handleMenuItemClick(item)}>
                  {item}
                </Button>
              </li>
            ))}
            ...
            <div className={`flex items-start gap-2 max-w-[80%] ${msg.sender === 'user' ? 'flex-row-reverse' : ''}`}>
              ...
              <div
                className={`p-3 rounded-lg text-sm ${
                  msg.sender === 'user' ? 'bg-primary-100 text-primary-800' : 'bg-neutral-100 text-neutral-800'
                }`}
            ...
            {msg.sender === 'agent' && msg.menu && (
              <div className="mt-2 w-full max-w-[80%]">
                <ul className="space-y-1">
                  {msg.menu.map((item) => (
                    <li key={item}>
                      <Button variant="outline" className="w-full text-left text-sm py-2" onClick={() => handleMenuItemClick(item)}>
                        {item}
                      </Button>
                    </li>
                  ))}
      ...
      <div className="p-4 border-t">
        <div className="text-[#00868B] flex items-center gap-2">
          ...
          />
          <div className="flex gap-1">
            <Button variant="ghost" size="icon" aria-label="Thumb survey" onClick={() => console.log('Thumb survey clicked')}>
            ...
            <label htmlFor="file-upload">
              ...
              />
              <Button variant="ghost" size="icon" asChild aria-label="Upload file">
                <span>
            ...
            <Button variant="ghost" size="icon" aria-label="Emoji picker" onClick={() => console.log('Emoji picker clicked')}>
